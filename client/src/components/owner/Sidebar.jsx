  ...
  return (
    ...
    max-w-13 md:max-w-60 w-full border-r border-borderColor text-sm'>
      ...
      <div className='group-relative'>
        <label htmlFor='image'>
          <img src={image ? URL.createObjectURL(image) : user?.image} alt='user Image'
        ...
        {image  && (
          ...
          bg-primary/10 text-primary cursor-pointer'>Save <img src={assets.check_icon} width={13} alt='' onClick={updateImage}/></button>
        )}
        <p className='mt-2 text-base max-md:hidden'>{user.name}</p>
        <div className='w-full'>
          {ownerMenuLinks.map((link,index)=>(
            <NavLink key={index} to={link.path} className={`relative flex items-center gap-2 w-full py-3 pl-2 first:mt-6 ${link.path=== location.pathname ? 
